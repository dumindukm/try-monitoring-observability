                ...
                .UseSerilog((context,configuration)=> {
                    configuration.Enrich.FromLogContext()
                                    ...
                                    .WriteTo.Elasticsearch(new Serilog.Sinks.Elasticsearch.ElasticsearchSinkOptions(new Uri(context.Configuration["ElasticConfiguration:Uri"]))
                                    {
                                        ...
                                        IndexFormat = $"{context.Configuration["ApplicationName"].ToLower().Replace(".", "-")}-logs-{context.HostingEnvironment.EnvironmentName?.ToLower().Replace(".", "-")}-{DateTime.UtcNow:yyyy-MM}",
